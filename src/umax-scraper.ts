...
export class UmaxScraperService {
  private baseUrl = 'https://uma-x.jp';
  ...
  async getRacePrediction(date: string, trackCode: string, raceNumber: string, raceId: string): Promise<UmaxRacePrediction> {
    const racePrediction: UmaxRacePrediction = {
      date: date,
      trackCode: trackCode,
   ...
   * そのレース一覧からレース予想を取得
   */
  async getRacePredictionMonthly(year: string, month: string): Promise<UmaxRacePrediction[]> {
    if (!this.page) return [];

    /* ---------------- 0) 月次ページへ ---------------- */
    const ym: string = `${year}${month.padStart(2, '0')}`;        // "202502"
    const monthlyUrl: string = `${this.baseUrl}/race/${ym}`;
    await this.page.goto(monthlyUrl, { waitUntil: 'load' });

    /* ---------------- 1) レース結果リンク一覧を取得 ----------------
       形式: <a class="top_race_menu" href="/race_result/5202501080120250223">…</a>
    ----------------------------------------------------------------- */
    const hrefs: string[] = await this.page.$$eval(
      'a.top_race_menu[href^="/race_result/"]',
      (as: Element[]): string[] =>
        Array.from(as).map(a => (a as HTMLAnchorElement).getAttribute('href') || '')
    );

    /* uniq にして raceId セットを作成 */
    const raceIdSet: Set<string> = new Set<string>();
    hrefs.forEach(href => {
      const id: string | undefined = href.split('/').pop();
      if (id && /^\d{19}$/.test(id)) raceIdSet.add(id);           // 19桁のみ対象
    });
    /* ---------------- 1.5) レース一覧を出力 ---------------- */
    ...
    const raceInfoList = Array.from(raceIdSet).map(raceId => {
      ...
      const trackName = getTrackName(paddedTrackCode);
    ...
    const predictions: UmaxRacePrediction[] = [];
    /* ---------------- 2) 各レース結果ページを順に処理 ---------------- */
    const errorRaces: { raceId: string, trackName: string, date: string, raceNumber: string, error: string }[] = [];
    ...
    for (const raceInfo of raceInfoList) {
      try {
        ...
        /* b) URL を組み立ててページ遷移 */
        const raceUrl: string = `${this.baseUrl}/race_result/${raceInfo.raceId}`;
        await this.navigateToRacePage(raceInfo.date, raceInfo.trackCode, raceInfo.raceNumber, raceUrl);

        /* c) 予想情報を収集 */
        const prediction: UmaxRacePrediction = {
          date: raceInfo.date,
   ...
   */
  async getRacePredictionByDate(date: string): Promise<UmaxRacePrediction[]> {
    if (!this.page) return [];
    ...
    /* ---------------- 0) 月次ページへ ---------------- */
    const ym: string = `${year}${month}`;        // "202502"
    const monthlyUrl: string = `${this.baseUrl}/race/${ym}`;
    await this.page.goto(monthlyUrl, { waitUntil: 'load' });
    ...
    hrefs.forEach(href => {
      const id: string | undefined = href.split('/').pop();
      if (id && /^\d{19}$/.test(id)) raceIdSet.add(id);
    });
  ...
  private async navigateToRacePage(date: string, trackCode: string, raceNumber: string, url: string): Promise<void> {
  ...
  // 注目馬の馬番を取得
  private async getFocusedHorseNumbers(): Promise<number[]> {
    if (!this.page) {
    ...
    try {
      ...
      const numberTexts = await this.page.$$eval('section.contents.tyumoku span.waku', spans =>
   ...
   * ① 左側テーブルから馬番配列を取得
   * ────────────────────────────── */
  private async getHorseNumbers(): Promise<number[]> {
    return this.page
      ? await this.page.$$eval(
          'div.table-midasi table tbody tr td.uma_no',
          tds => tds.map(td => Number((td.textContent || '').trim()))
        )
      : [];
  }
   ...
   * ────────────────────────────── */
  private async getColumnTextsByHeader(
    headerKeyword: string,
    firstTextOnly: boolean = false
  ): Promise<string[]> {
    if (!this.page) return [];

    // ★ page.$eval の第3引数は 1 個だけ ⇒ [kw, firstOnly] のタプルを渡す
    // page.$evalの引数の問題を修正
    const colTexts: string[] = await this.page.evaluate(
      ({ selector, kw, firstOnly }) => {
        const tbl = document.querySelector(selector) as HTMLTableElement;
        if (!tbl) return [];

        /* 1) ヘッダー列番号を取得 */
        const headers: HTMLTableCellElement[] =
          Array.from(tbl.querySelectorAll('thead th'));
        const idx: number = headers.findIndex(th =>
          (th.textContent || '').replace(/\s+/g, '').includes(kw)
        );
        if (idx === -1) {
        ...
        return rows.map((tr: HTMLTableRowElement): string => {
          const cell = tr.querySelectorAll('td')[idx] as HTMLTableCellElement;
          if (!cell) return '';
   ...
   * -------------------------------------------------------
   */
  private async getTopNByColumn(
    headerKeyword: string,
    parser: (txt: string) => number,
    topN: number,
    ascending = true
  ): Promise<number[]> {
    ...
    const pairs: { horse: number; value: number }[] = [];
    const len = Math.min(horses.length, colTexts.length);

    for (let i = 0; i < len; i++) {
      const h = horses[i];
      const v = parser(colTexts[i]);
      if (!isNaN(h) && !isNaN(v)) pairs.push({ horse: h, value: v });
    }
  ...
  // タイム偏差上位3頭の馬番を取得
  private async getTimeDeviationTop3(): Promise<number[]> {
    // "mm:ss.s" → 秒数 に変換
   ...
   * 上位3頭の馬番を返す
   */
  private async getLastSpurtDeviationTop3(): Promise<number[]> {
    const toNum = (t: string): number =>
      parseFloat(t.replace(/\s+/g, ''));           // 例 "37.2"
   ...
   */
  private async getSpValueTop5(): Promise<number[]> {
    if (!this.page) return [];
    ...
    const spTexts = await this.getColumnTextsByHeader(
      'SP値',
      true
    );

    /* c) 数値化＆ペア化 */
    const pairs: { horse: number; sp: number }[] = [];
    const len = Math.min(horses.length, spTexts.length);
    for (let i = 0; i < len; i++) {
      const v = parseFloat(spTexts[i]);
      if (!isNaN(horses[i]) && !isNaN(v)) pairs.push({ horse: horses[i], sp: v });
    }
    /* d) 大きい順に並べ替え → 上位5頭 */
  ...
  private async getAgValueTop5(): Promise<number[]> {
    if (!this.page) return [];
    ...
    const agTexts = await this.getColumnTextsByHeader(
      'AG値',
      true
    );

    /* c) 数値化＆ペア化 */
    const pairs: { horse: number; ag: number }[] = [];
    const len = Math.min(horses.length, agTexts.length);
    for (let i = 0; i < len; i++) {
      const v = parseFloat(agTexts[i]);
      if (!isNaN(horses[i]) && !isNaN(v)) pairs.push({ horse: horses[i], ag: v });
    }
    /* d) 大きい順に並べ替え → 上位5頭 */
  ...
  private async getSaValueTop5(): Promise<number[]> {
    if (!this.page) return [];
    ...
    const saTexts = await this.getColumnTextsByHeader(
      'SA値',
      true
    );

    /* c) 数値化＆ペア化 */
    const pairs: { horse: number; sa: number }[] = [];
    const len = Math.min(horses.length, saTexts.length);
    for (let i = 0; i < len; i++) {
      const v = parseFloat(saTexts[i]);
      if (!isNaN(horses[i]) && !isNaN(v)) pairs.push({ horse: horses[i], sa: v });
    }
    /* d) 大きい順に並べ替え → 上位5頭 */
   ...
   * KI値が高い順トップ3の馬番を取得
   */
  private async getKiValueTop3(): Promise<number[]> {
    const toNum = (t: string): number => parseInt(t, 10)
