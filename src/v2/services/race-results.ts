...
import { RaceResultScraper } from '../scrapers/race-results/scraper';
import { RaceResultsEndpoint } from '../api/endpoints/race-results';
import { createApiClient } from '../api/client';
import { mapRaceResultsToApi } from '../mappers/race-results';
import { logger } from '../utils/logger';

/**
 * レース結果API実行
 */
export async function executeRaceResults(options: ExecutionOptions): Promise<boolean> {
  // 日付を文字列に変換
  const dateStr = Array.isArray(options.date) ? options.date[0] : options.date;
  logger.info(`レース結果API実行開始: ${dateStr}`);

  const scraper = new RaceResultScraper();
  const apiClient = createApiClient();
  const endpoint = new RaceResultsEndpoint(apiClient);
  ...
  try {
    ...
    for (const trackCode of options.trackCodes) {
      try {
        logger.info(`レース結果スクレイピング: ${trackCode}`);
        const result = await scraper.scrapeRaceResults(dateStr, trackCode);
        
        if (!result.success || !result.data || result.data.length === 0) {
          logger.warn(`レース結果スクレイピング失敗: ${trackCode} - ${result.error}`);
          allSuccess = false;
          continue;
        }
        ...
        // 即座にAPI形式に変換
        const apiRequests = mapRaceResultsToApi(result.data);
    ...
    logger.error('レース結果API実行中にエラーが発生しました', error);
    return false;
  }
}