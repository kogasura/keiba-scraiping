...
// Scraping API Type Definitions
// 自動生成: backend/resources/types/scraping-api-types.d.ts
// スクレイピングアプリから Laravel API (/api/v1/*) へ送るリクエスト／レスポンス型をまとめています。
...
// 2. /v1/predictions
// ==============================================
export type RankArray<N extends number> = (HorseNumber | null)[] & { length: N };
...
// ==============================================
// 6. /index-images/list/{date} (画像リスト取得)
// ==============================================
export interface IndexImageListItem {
  race_no: number;
  image_path: string;
  is_processed: boolean;
}

export interface IndexImageListResponse {
  success: true;
  date: string;
  venue: TrackCode;
  images: IndexImageListItem[];
}

// ==============================================
// 7. /ai-index-urls/{date} (AI指数URL取得)
// ==============================================
export interface AiIndexUrlsResponse {
  success: true;
  date: string;
  urls: Record<TrackCode, string>;
}

// ==============================================
// 8. /ai-index-urls (AI指数URL保存)
// ==============================================
export interface AiIndexUrlSaveRequest {
  date: `${number}-${number}-${number}`;
  venue: TrackCode;
  ai_index_url: string;
}

export interface AiIndexUrlSaveResponse {
  success: true;
  message: string;
  data: {
    id: number;
...
export type EndpointRequest =
  ...
  | { url: '/api/ai-index-urls'; body: AiIndexUrlSaveRequest };

export type GetEndpointResponse =
  | { url: '/api/index-images/list/{date}'; response: IndexImageListResponse }
  | { url: '/api/ai-index-urls/{date}'; response: AiIndexUrlsResponse };
