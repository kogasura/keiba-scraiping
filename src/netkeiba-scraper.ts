...
import { NetkeibaCPPrediction, NetkeibaDataAnalysis, NetkeibaDataAnalysisSimple, RaceInfo, TimeIndex } from './types';
...
import { getTrackCode } from './consts';
...
export class NetkeibaScraper {
  ...
  async login(): Promise<boolean> {
    try {
      ...
      await this.page!.fill('input[name="login_id"]', CONFIG.NETKEIBA.USERNAME);
      await randomDelay(500, 800);
      await this.page!.fill('input[name="pswd"]', CONFIG.NETKEIBA.PASSWORD);
            ...
            for (const row of raceRows) {
              try {
                ...
                allRaces.push({
                  ...
                  track_type: trackType as '芝' | 'ダート' | '障害',
                  distance: distance || 0,
   ...
   */
  async getRaceDetail(raceId: number, date: string): Promise<RaceInfo> {
    if (!this.page) {
      throw new Error('ブラウザが初期化されていません');
    }
   ...
   */
  private async getFinishedRaceDetail(raceId: number, date: string): Promise<RaceInfo> {
    // レース基本情報の取得
    const raceInfo = await this.page!.evaluate(() => {
      ...
      // 賞金
      let prizeMoney = 0;
      const prizeElement = Array.from(document.querySelectorAll('.RaceData02 span')).find(span => 
        span.textContent?.includes('本賞金:')
      );
      if (prizeElement) {
    ...
    return   {
      date: date,
      trackCode: getTrackCode(raceInfo.venue),
   ...
   */
  private async getUpcomingRaceDetail(raceId: number, date: string): Promise<RaceInfo> {
    // レース基本情報の取得
    const raceInfo = await this.page!.evaluate(() => {
      // レース名 
      const raceTitleElement = document.querySelector('.RaceName');
