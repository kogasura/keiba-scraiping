...
import { compareTrackCode, getTrackName } from './consts';
 ...
 */
export function saveAnalysisToExcel(analysisItems: AnalysisItem[], filename: string): void {
  ...
  const rows = sortedAnalysisItems.map(item => {
    const row: Record<string, any> = {
      ...
      "会場": getTrackName(item.trackCode),
    ...
    // AI指数
    if (item.ai_ranks && item.ai_ranks[0]) row["AI1"] = item.ai_ranks[0];
    if (item.ai_ranks && item.ai_ranks[1]) row["AI2"] = item.ai_ranks[1];
    if (item.ai_ranks && item.ai_ranks[2]) row["AI3"] = item.ai_ranks[2];
    if (item.ai_ranks && item.ai_ranks[3]) row["AI4"] = item.ai_ranks[3];
    if (item.ai_ranks && item.ai_ranks[4]) row["AI5"] = item.ai_ranks[4];
    ...
    // 高期待リスト
    if (item.high_expectation_list1 !== undefined) row["高期待リスト1"] = item.high_expectation_list1 ? "○" : "";
    if (item.high_expectation_list2 !== undefined) row["高期待リスト2"] = item.high_expectation_list2 ? "○" : "";
