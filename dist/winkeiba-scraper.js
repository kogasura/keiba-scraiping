...
exports.WinkeibaScraperService = void 0;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const utils_1 = require("./utils");
const playwright_utlis_1 = require("./playwright-utlis");
const config_1 = require("./config");
class WinkeibaScraperService {
    constructor() {
        this.baseUrl = config_1.CONFIG.WINKEIBA.BASE_URL;
        this.browser = null;
        this.context = null;
        this.page = null;
     ...
     */
    async getRaceList(date, track_codes) {
        try {
            if (!this.page) {
                await this.init();
            }
            ...
            await (0, utils_1.randomDelay)(2000, 4000);
            let allRaceList = [];
            for (const trackCode of track_codes) {
                ...
                try {
                    ...
                    fs.writeFileSync(`debug/racelist-${date}-${trackCode}.html`, dump, 'utf-8');
                }
     ...
     */
    async getRaceMarks(date, trackCode, raceNumber) {
        try {
            if (!this.page) {
                await this.init();
            }
            ...
            try {
                ...
                // 新聞印情報をページから抽出
                const horseMarks = await this.page.evaluate(() => {
                    const horseMarkArray = {
                        date: '',
                        trackCode: '',
                        raceNumber: '',
                        marks: []
    ...
    async login() {
        try {
            ...
            // ログイン情報入力
            await (0, utils_1.randomDelay)(400, 800);
            await this.page.fill('input[type="text"].form-text', config_1.CONFIG.WINKEIBA.USERNAME);
            await this.page.fill('input[type="password"].form-text', config_1.CONFIG.WINKEIBA.PASSWORD);
     ...
     * @param raceNumber レース番号
     */
    async getAnalysisData(date, trackCode, raceNumber) {
        try {
            if (!this.page) {
            ...
            (0, playwright_utlis_1.ensureDirectoryExists)(htmlDir);
     ...
     * @returns レース結果情報
     */
    async getRaceResults(date, RacetrackCd) {
        try {
            // URLを構築
            const DOR = date;
            ...
            // ---------- 汎用ユーティリティ ----------
            const getTextSafe = async (root, selector, transform = (t) => t.trim()) => {
                try {
                    if (!root)
                        return '';
            ...
            for (const section of raceSections) {
                ...
                if (rankingTable) {
                    ...
                    if (rankingRows[0])
                        results.first_place = await parseRankRow(rankingRows[0]);
                    if (rankingRows[1])
                        results.second_place = await parseRankRow(rankingRows[1]);
                    if (rankingRows[2])
                        results.third_place = await parseRankRow(rankingRows[2]);
                }
                // 配当情報を取得
                ...
                if (payoutTable) {
                    ...
                    // 文字ラベルで行を取る
                    const findRow = async (keyword) => (await Promise.all(rows.map(async (r) => ((await getTextSafe(r, 'td.line')) || '').includes(keyword) ? r : null))).find(Boolean) ?? null;
                    const parseCombo = async (row) => (await row.$$eval('i.icon-umaban', (els) => els.map((e) => (e.textContent || '').trim()).join('-'))) || '';
                    const parsePayout = async (row) => parseInt((await row.$eval('td.-right', (el) => (el.textContent || '').replace(/[^0-9]/g, ''))) || '0') || 0;
                    const parsePopularity = async (row) => parseInt(((await getTextSafe(row, 'td.-center')).match(/(\d+)/)?.[1] || '0').trim()) || 0;
                    /* 単勝 */
                    ...
                    if (winRow) {
                        ...
                        results.win.popularity =
                            parseInt(((await getTextSafe(winRow, 'td.-center')).match(/(\d+)/)?.[1] || '0')) || 0;
                    }
                    /* 複勝（連続 1〜3 行） */
                    ...
                    if (placeRow) {
                        ...
                        while (idx < rows.length && placeArr.length < 3) {
                            ...
                            if (lineCls && !lineCls.includes('複勝') && placeArr.length)
                                break;
                            placeArr.push({
                                horse_number: parseInt(await getTextSafe(row, 'i.icon-umaban')) || 0,
                                payout: parseInt((await getTextSafe(row, 'td.-right')).replace(/[^0-9]/g, '')) || 0,
                                popularity: parseInt(((await getTextSafe(row, 'td.-center')).match(/(\d+)/)?.[1] || '0')) || 0
                            });
                            idx++;
                        }
                    ...
                    if (trifecta) {
                        const comboText = await trifecta.$$eval('i.icon-umaban', (els) => els.map((e) => (e.textContent || '').trim()).join('→'));
                        results.trifecta = {
...
exports.WinkeibaScraperService = WinkeibaScraperService;
